...
- **Carved HWL coast (same 3D tab).** USGS OFR 2010-1119 `LongIsland_shorelines` clipped to Montauk is `data/usgs_hwl_montauk.geojson` (Himmelstoss et al. 2010). Surveyed years only: 1830, 1870, 1892, 1933, 1938, 1962, 1979, 1983, 1988, 2000. 2014 is the measured NOAA NGS DEM, not a HWL. South/Point decade worlds (1871–2021) are linear in time along the official LT transects, or held at 2000 after the last survey — labeled **Modeled from USGS high-water-line trend. Not a surveyed shoreline.** `data/coast_meshes.json` stores the transect-aligned HWL vs the 2000 line (2014 waterline proxy) plus 2014 `zCut` heights seaward of each HWL. The 3D tab draws a water plane, hides 2014 land/hillshade seaward of that year’s HWL, adds a sand terrace only where the HWL is seaward, cuts lost land to water, and a till/cliff face inland of the beach. Not a historic DEM. This is a mesh, not a GeoJSON paint on one globe. 1891 and 1991 exist elsewhere on Long Island and are not planted as Montauk surveys. North HWL is only the two exact segments (1933 t5079, 2000 lidar Point stub). No Soundview / harbor waterline is invented; no peninsula loop. 1871 keeps its Mode C still; the waterline is modeled from 1870–1892. Van Scoyoc 100–200 ft and the Ditch 20,000 cy / +16 ft figures stay captions, not modeled widths. Inland 2014 relief does not change.
...
- USGS OFR 2010-1119 historic high-water lines, Montauk bbox only — `data/usgs_hwl_montauk.geojson` from `LongIsland_shorelines.shp` (Himmelstoss et al. 2010). Decade reconstructions for the 3D tab are `data/usgs_hwl_worlds.geojson` and are labeled modeled, not surveyed.
